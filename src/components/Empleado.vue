<template>
  <v-card elevation="5" outlined shaped>
    ...
    <v-dialog v-model="editarEmpleadoModal" max-width="1000px">
      <v-card elevation="5" outlined shaped>
        <v-card-title>
          <span>
            <h5>TIPO DE EMPLEADO</h5>
          ...
          <v-row>
            ...
            <v-col cols="12" md="6">
              <v-text-field v-model="nombre" :counter="50" :rules="nombreRules" @input="nombre = nombre.toUpperCase()"
                label="NOMBRE DE EMPLEADO" required></v-text-field>
            </v-col>
            <v-col cols="12" md="3"> </v-col>
            <v-col cols="12" md="3"> </v-col>
            <v-col cols="12" md="6">
...
export default {
  ...
  methods: {
    ...
    regempleado() {
      this.verificarempleado(this.id_empleado, this.id_tipo_empleado, this.id_sede, this.usuario, this.telefonopost, this.nombre);
      //this.savemateplan(this.idpla,this.idmateall,this.idnivel,this.id_sede,this.creditos,this.hrst,this.hrsp);
    },
    async verificarempleado(id_empleado, id_tipo_empleado, id_sede, usuario, telefonopost, nombre) {
      let me = this;
      await axios.get("/empleado/verificarempleado/" + this.nombre + "," + this.id_empleado + "," + this.id_sede)
        .then(function (response) {
          ...
          if (response.data.resultado == null) {
            ...
            me.registroEmpleado(me.id_empleado, me.id_tipo_empleado, me.id_sede, me.usuario, me.telefonopost, me.nombre);
...
*/
    ...
    seleccionarEmpleado(item) {
      this.id_empleado = item.idbloque;
      this. = item.bloque;
